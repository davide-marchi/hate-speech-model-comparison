...
\author{\IEEEauthorblockN{Davide Marchi}\\\IEEEauthorblockA{Email: d.marchi5@studenti.unipi.it}}
\maketitle
...
Hate speech detection is commonly framed as a text classification task. In practical settings, model choice should consider not only effectiveness (e.g., macro-F1) but also efficiency and environmental impact. We study this trade-off across five representative approaches, from simple keyword rules to modern pretrained models. The complete code and artifacts are available at: \href{https://github.com/davide-marchi/hate-speech-model-comparison}{github.com/davide-marchi/hate-speech-model-comparison}.

\section{Data}
We use the Twitter hate speech dataset (training split) as provided on Kaggle \cite{kaggle_twitter_hate_speech}. Table~\ref{tab:data} summarizes corpus-level statistics; class distribution is in Table~\ref{tab:dist}. We apply standard text cleaning (URLs, mentions, lowercasing) consistent with reproducible baselines.

\begin{table}[!t]
\caption{Dataset overview (from processed training data).}
\label{tab:data}
\centering
\csvreader[
    tabular=lccc,
    table head=\toprule n\_records & n\_classes & avg\_tokens & median\_tokens\\\midrule,
    late after line=\\,
...
\begin{table}[!t]
\caption{Label distribution in the training data.}
\label{tab:dist}
\centering
\csvreader[
    tabular=rrr,
    table head=\toprule label & count & percent \\ \midrule,
    late after line=\\,
    table foot=\bottomrule
...
\begin{itemize}
    ...
    \item \textbf{SVM + TF--IDF}: a linear SVM on word/character TF--IDF features, a strong classical baseline for text classification \cite{joachims1998text,salton1988term}.
    \item \textbf{MiniLM + Logistic Regression}: sentence embeddings from Sentence-Transformers \texttt{all-MiniLM-L6-v2} \cite{reimers2019sentence,wang2020minilm} followed by logistic regression.
    \item \textbf{LSTM}: a recurrent model over token sequences \cite{hochreiter1997long}.
    \item \textbf{Zero-shot (DistilBERT-MNLI)}: \texttt{typeform/distilbert-base-uncased-mnli} \cite{sanh2019distilbert} used with an NLI-based zero-shot pipeline \cite{wolf2020transformers}.
\end{itemize}
Hyperparameters are selected by cross-validation where applicable, and we report the best validation score together with test metrics (see Table~\ref{tab:summary}).

\section{Experimental Setup}
All runs are performed on CPU only for consistency across models and to ensure comparable emissions estimates. We track both \emph{time} and \emph{carbon emissions (kg~CO$_2$e)} during training and testing. Emissions reflect runtime and estimated power draw, which depends on CPU utilization; therefore, we emphasize emissions as an efficiency metric rather than wall-clock time alone \cite{strubell2019energy,lacoste2019quantifying}.
...
\begin{table}[!t]
\caption{Effectiveness and emissions summary. Values pulled directly from CSV results.}
\label{tab:summary}
\centering
\csvreader[
    tabular=lcccc,
    table head=\toprule model & macro-F1 & accuracy & test~emissions~(kg) & train~emissions~(kg) \\\midrule,
    late after line=\\,
    table foot=\bottomrule
]{summary_table.csv}{}{
    \modelname{\csvcoli} & \csvcolii & \csvcoliii & \csvcoliv & \csvcolv
}
\end{table}
...
\csvreader[
    ...
    \modelname{\csvcoli} & \modelname{\csvcolii} & \csvcolviii & \csvcolix
}
\end{table}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{../results/svm_tfidf/analysis/confusion_matrix.png}
...
T. Wolf et al., ``Transformers: State-of-the-Art Natural Language Processing,'' in EMNLP: System Demonstrations, 2020.
